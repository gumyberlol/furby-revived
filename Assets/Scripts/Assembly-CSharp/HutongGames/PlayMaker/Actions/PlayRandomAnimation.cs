...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class PlayRandomAnimation : FsmStateAction
	{
		...
		private void DoPlayAnimation(string animName)
		{
			...
			anim = ownerDefaultTarget.animation[animName];
			if (anim == null)
			{
			...
			if (value < 0.001f)
			{
				ownerDefaultTarget.animation.Play(animName, playMode);
			}
			else
			{
				ownerDefaultTarget.animation.CrossFade(animName, value, playMode);
			}
		...
		private void StopAnimation()
		{
			...
			{
				ownerDefaultTarget.animation.Stop(anim.name);
			}
		}
	}
}
