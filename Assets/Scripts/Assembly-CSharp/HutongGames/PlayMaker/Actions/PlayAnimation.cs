...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class PlayAnimation : FsmStateAction
	{
		...
		private void DoPlayAnimation()
		{
			...
			anim = ownerDefaultTarget.animation[animName.Value];
			if (anim == null)
			{
			...
			if (value < 0.001f)
			{
				ownerDefaultTarget.animation.Play(animName.Value, playMode);
			}
			else
			{
				ownerDefaultTarget.animation.CrossFade(animName.Value, value, playMode);
			}
		...
		private void StopAnimation()
		{
			...
			{
				ownerDefaultTarget.animation.Stop(animName.Value);
			}
		}
	}
}
